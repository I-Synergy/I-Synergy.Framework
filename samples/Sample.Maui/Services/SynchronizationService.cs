...
internal class SynchronizationService : ISynchronizationService
{
    ...
    public SynchronizationService(
        ...
        IOptions<ClientApplicationOptions> configurationOptions)
    {
        _context = context;
        ...
        if (_synchronizationSettings is not null && ((RoamingSettings)_settingsService.RoamingSettings).IsSynchronizationEnabled)
        {
            ...
            {
                _messengerService.Send(new SyncSessionStateChangedMessage(state));
            };
        }
    }
    ...
    public async Task SynchronizeAsync(SyncType syncType, string scopeName = SyncOptions.DefaultScopeName, CancellationToken cancellationToken = default)
    {
        ...
        if (_synchronizationSettings is not null && IsActive)
        {
            ...
            {
                _messengerService.Send(new SyncProgressMessage(args.ProgressPercentage));

                if (result == string.Empty)
                    _messengerService.Send(new SyncMessage(args.Message));
            });
            ...
            _messengerService.Send(new SyncMessage(result));
        }
    }
}
