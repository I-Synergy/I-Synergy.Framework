...
public class DownloadFileService : IDownloadFileService
{
    ...
    public async Task DownloadFileAsync(string folder, string filename, byte[] file)
    {
        ...
        {
#if WINDOWS
            var storageFile = await StorageFile.GetFileFromPathAsync(savedFile.FullPath);
            await Launcher.Default.OpenAsync(new OpenFileRequest("", new ReadOnlyFile(storageFile.Path)));
#endif
        }
    }
}
