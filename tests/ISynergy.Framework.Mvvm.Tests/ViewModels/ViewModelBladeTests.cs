...
public class ViewModelBladeTests
{
    ...
    public class TestObservableBladeViewModel : ViewModelBlade<TestObservableEntity>
    {
 public int PropertyChangedCallCount { get; private set; }

        public TestObservableBladeViewModel(ICommonServices commonServices, ILogger<TestObservableBladeViewModel> logger)
: base(commonServices, logger)
        {
      PropertyChanged += (s, e) =>
        {
...
Assert.IsTrue(closedInvoked);
    ...
    public void Cleanup_ClearsSelectedItemAndCommands()
 {
  ...
  Assert.AreEqual(default, viewModel.SelectedItem);
    ...
    {
  // Arrange
        var viewModel = new TestObservableBladeViewModel(_mockCommonServices.Object, new Mock<ILogger<TestObservableBladeViewModel>>().Object);
