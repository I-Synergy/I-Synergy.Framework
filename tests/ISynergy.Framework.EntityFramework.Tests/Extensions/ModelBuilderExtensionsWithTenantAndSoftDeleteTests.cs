...
[TestClass]
public class ModelBuilderExtensionsWithTenantAndSoftDeleteTests
{
    private TestDataContextWithTenantFilterAndSoftDeleteFilter _context;
    ...
    public ModelBuilderExtensionsWithTenantAndSoftDeleteTests()
    {
        var options = new DbContextOptionsBuilder<TestDataContext>()
            .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
            .Options;

        _context = new TestDataContextWithTenantFilterAndSoftDeleteFilter(options, () => _testTenantId);
        SeedDatabase();
    }
    ...
    [TestMethod]
    public async Task CombinedFilters_ShouldApplyBothFilters()
    {
        // Act
        var entities = await _context.TestTenantEntities.ToListAsync();
        ...
        // Assert
        Assert.AreEqual(1, entities.Count);
        Assert.IsTrue(entities.All(e => e.TenantId == _testTenantId && !e.IsDeleted));
    }
