...
public class ExceptionHandlerService : IExceptionHandlerService
{
    ...
    private async Task HandleExceptionAsync(Exception exception, bool isStartupExceptionReplay)
    {
        ...
        try
        {
            ...
            var uiTask = EnsureUiThreadAsync(async () =>
            {
                try
                {
    ...
    // Helper method to ensure code runs on UI thread with better error handling
    private async Task EnsureUiThreadAsync(Func<Task> action)
    {
        try
        {
            // Get the current window's dispatcher
            var dispatcher = Microsoft.UI.Dispatching.DispatcherQueue.GetForCurrentThread();

            if (dispatcher == null)
            {
                // Try to get the main window's dispatcher if available
                if (Microsoft.UI.Xaml.Application.Current is Application app &&
                    Application.MainWindow?.DispatcherQueue != null)
                    dispatcher = Application.MainWindow.DispatcherQueue;
            }
            ...
            else
            {
                ...
                bool queued = dispatcher.TryEnqueue(DispatcherQueuePriority.Normal, async () =>
                {
                    try
                    {
                        await action();
                    ...
                    {
                        _logger.LogError(ex, "Error in dispatcher callback");
                ...
                if (!queued)
                {
                    _logger.LogWarning("Failed to queue UI operation on dispatcher");
                ...
                else
                {
                    ...
                    if (completedTask == timeoutTask)
                    {
                        _logger.LogWarning("Dispatcher operation timed out");
        ...
        catch (Exception ex)
        {
            ...
            {
                _logger.LogError(actionEx, "Error in fallback action execution");
            }
        }
    }
