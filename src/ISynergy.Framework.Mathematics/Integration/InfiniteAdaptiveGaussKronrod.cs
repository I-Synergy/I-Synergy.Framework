namespace ISynergy.Framework.Mathematics.Integration
{
    ...
    ///         Kronrod integration is
    ///         <c>
    ///             qagi(f,bound,inf,epsabs,epsrel,result,abserr,neval,
    ///             ier,limit,lenw,last,iwork,work)
    ///         </c>
    ///         . The original source code is in the public
    ...
    ///             <item>
    ///                 <description>
    ///                     <a href="http://en.wikipedia.org/wiki/Adaptive_quadrature">
    ...
    ///             <item>
    ///                 <description>
    ///                     <a href="http://en.wikipedia.org/wiki/QUADPACK">
    ...
    ///             <item>
    ///                 <description>
    ///                     <a href="http://www.netlib.no/netlib/quadpack/qagi.f">
        ...
        INumericalIntegration<InfiniteAdaptiveGaussKronrodStatus>
    {
        ...
        /// </summary>
        public int Subintervals => iwork.Length;
        ...
        public double ToleranceRelative
        {
            ...
            set
            {
                var limit = Math.Max(Constants.SingleEpsilon * 50.0, 5e-15);
        ...
        /// </summary>
        public int FunctionEvaluations => evaluations;
            ...
            double[] dwork)
        {
            ...
            if (limit < 1 || lenw < limit << 2) goto L10;
            ...
            double* rlist, double* elist, int* iord, ref int last)
        {
            ...
            var res3la_ = new double[3 + 1];
            var rlist2_ = new double[52 + 1];
            ...
            fixed (double* rlist2 = rlist2_)
            {
                ...
                if (abserr <= epmach * 100.0 * defabs && abserr > errbnd) ier = 2;
                if (limit == 1) ier = 1;
                if (ier != 0 || abserr <= errbnd && abserr != resabs || abserr == 0.0) goto L130;

                /*           initialization */
                /*           -------------- */

                uflow = float.MinValue;
                oflow = float.MaxValue;
                rlist2[0] = result;
                errmax = abserr;
                maxerr = 1;
                area = result;
                ...
                iroff3 = 0;
                ksgn = -1;
                if (dres >= (1.0 - epmach * 50.0) * defabs) ksgn = 1;
                ...
                for (last = 2; last <= i__1; ++last)
                {
                    ...
                    if (Math.Abs(rlist[maxerr] - area12) > Math.Abs(area12) * 1e-5 || erro12 < errmax * 0.99) goto L10;
                    if (extrap) ++iroff2;
                    if (!extrap) ++iroff1;
                    L10:
                    ...
                    var alpha = Math.Max(Math.Abs(a1), Math.Abs(b2));
                    var beta = (epmach * 100.0 + 1.0) * (Math.Abs(a2) + uflow * 1e3);
                    if (alpha <= beta) ier = 4;
                ...
                L30:
                    ...
                    if (Math.Abs(b1 - a1) > small) erlarg += erro12;
                    if (extrap) goto L40;

                    /*           test whether the interval to be bisected next is the */
                    /*           smallest interval. */

                    if (Math.Abs(blist[maxerr] - alist__[maxerr]) > small) goto L90;
                    extrap = true;
                    nrmax = 2;
                L40:
                ...
                L60:
                    ...
                    if (ktmin > 5 && abserr < errsum * .001) ier = 5;
            ...
            L100:
                ...
                goto L110;
            L105:
                if (abserr / Math.Abs(result) > errsum / Math.Abs(area)) goto L115;

                /*           test on divergence */

                L110:
                /* Computing MAX */
                if (ksgn == -1 && Math.Max(Math.Abs(result), Math.Abs(area)) <= defabs * 0.01) goto L130;
            ...
            double b, ref double result, ref double abserr, ref double resabs, ref double resasc)
        {
            ...
            int j;
            double fc;
            var fv1_ = new double[7 + 1];
            var fv2_ = new double[7 + 1];
            ...
            fixed (double* fv2 = fv2_)
            {
                ...
                if (resabs > uflow / (epmach * 50.0)) abserr = Math.Max(epmach * 50.0 * resabs, abserr);
            }
