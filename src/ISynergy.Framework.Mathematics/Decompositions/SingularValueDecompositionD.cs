...
namespace ISynergy.Framework.Mathematics.Decompositions
{
    ...
    public sealed class SingularValueDecompositionD : ICloneable, ISolverMatrixDecomposition<decimal>
    {
        ...
        private decimal? lndeterminant;
        private decimal? lnpseudoDeterminant;
        private int m;
        private int n;
        private decimal? pseudoDeterminant;
            ...
            bool computeLeftSingularVectors, bool computeRightSingularVectors, bool autoTranspose, bool inPlace)
        {
            ...
            Diagonal = new decimal[ni];
            LeftSingularVectors = new decimal[m, nu];
            RightSingularVectors = new decimal[n, n];

            var e = new decimal[n];
            var work = new decimal[m];
            ...
            var pp = p - 1;
            var iter = 0;
            var eps = Constants.DecimalEpsilon;
            while (p > 0)
            {
                int k, kase;
        ...
        /// </summary>
        public decimal Condition => Diagonal[0] / Diagonal[Math.Max(m, n) - 1];

        /// <summary>
        ///     Returns the singularity threshold.
        /// </summary>
        public decimal Threshold => Constants.DecimalEpsilon * Math.Max(m, n) * Diagonal[0];

        /// <summary>
        ///     Returns the Two norm.
        /// </summary>
        public decimal TwoNorm => Diagonal[0];
        ...
        /// </summary>
        public decimal[] Diagonal { get; private set; }
        ...
        /// </summary>
        public decimal[,] RightSingularVectors { get; private set; }

        /// <summary>
        ///     Returns the U matrix of Singular Vectors.
        /// </summary>
        public decimal[,] LeftSingularVectors { get; private set; }
        ...
        public decimal LogDeterminant
        {
            get
            {
                if (!lndeterminant.HasValue)
                {
                    double det = 0;
                    for (var i = 0; i < Diagonal.Rows(); i++)
                        det += Math.Log((double)Diagonal[i]);
        ...
        public decimal[,] Inverse()
        {
            ...
            var X = new decimal[vrows, Diagonal.Rows()];
            for (var i = 0; i < vrows; i++)
        ...
        public decimal[,] SolveForDiagonal(decimal[] value)
        {
            ...
            //(V x L*) x Ut x Y
            decimal[,] VL = Matrix.Dot(RightSingularVectors, Ls);
        ...
        public object Clone()
        {
            ...
            svd.m = m;
            svd.n = n;
            svd.Diagonal = (decimal[])Diagonal.Clone();
            svd.Ordering = (int[])Ordering.Clone();
            svd.swapped = swapped;
            if (LeftSingularVectors != null)
                svd.LeftSingularVectors = (decimal[,])LeftSingularVectors.MemberwiseClone();
            if (RightSingularVectors != null)
                svd.RightSingularVectors = (decimal[,])RightSingularVectors.MemberwiseClone();
