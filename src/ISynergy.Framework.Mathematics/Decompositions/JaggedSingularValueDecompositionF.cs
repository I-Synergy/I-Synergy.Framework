...
namespace ISynergy.Framework.Mathematics.Decompositions
{
    ...
    ///         <see cref="JaggedSingularValueDecompositionF(float[][], bool, bool, bool)">autoTranspose</see>
    ...
    /// </remarks>
    [Serializable]
    public sealed class JaggedSingularValueDecompositionF : ICloneable, ISolverArrayDecomposition<float>
    {
        private const float eps = 2 * Constants.SingleEpsilon;
        private const float tiny = Constants.SingleSmall;
        private float? determinant;

        private float[][] diagonalMatrix;
        private float? lndeterminant;
        private float? lnpseudoDeterminant;
        private int m;
        private int n;
        private float? pseudoDeterminant;
        ...
        /// </param>
        public JaggedSingularValueDecompositionF(float[][] value)
            ...
            bool computeLeftSingularVectors, bool computeRightSingularVectors, bool autoTranspose, bool inPlace)
        {
            ...
            if (m < n) // Check if we are violating JAMA's assumption
            {
                if (!autoTranspose) // Yes, check if we should correct it
                {
                    ...
                    // Proceed anyway
                    a = inPlace ? value : (float[][])value.MemberwiseClone();
                }
                else
                {
            ...
            Diagonal = new float[ni];
            LeftSingularVectors = new float[m][];
            for (var i = 0; i < LeftSingularVectors.Length; i++)
                LeftSingularVectors[i] = new float[nu];

            RightSingularVectors = new float[n][];
            for (var i = 0; i < RightSingularVectors.Length; i++)
                RightSingularVectors[i] = new float[n];

            var e = new float[n];
            var work = new float[m];
            ...
            var pp = p - 1;
            var iter = 0;
            var eps = Constants.SingleEpsilon;
            while (p > 0)
            {
                int k, kase;
                ...
                for (k = p - 2; k >= -1; k--)
                {
                    ...
                    if (Math.Abs(e[k]) <= alpha || float.IsNaN(e[k]))
                    {
                        e[k] = 0;
                        break;
                    }
        ...
        /// </summary>
        public float Condition => Diagonal[0] / Diagonal[Math.Max(m, n) - 1];

        /// <summary>
        ///     Returns the singularity threshold.
        /// </summary>
        public float Threshold => Constants.SingleEpsilon * Math.Max(m, n) * Diagonal[0];

        /// <summary>
        ///     Returns the Two norm.
        /// </summary>
        public float TwoNorm => Diagonal[0];
        ...
        /// </summary>
        public float[][] RightSingularVectors { get; private set; }

        /// <summary>
        ///     Returns the U matrix of Singular Vectors.
        /// </summary>
        public float[][] LeftSingularVectors { get; private set; }
        ...
        public float[][] Solve(float[][] value)
        {
            ...
            var Ls = new float[scols][];
            for (var i = 0; i < Diagonal.Length; i++)
            {
                Ls[i] = new float[scols];
            ...
            var VLU = Jagged.Create<float>(vrows, urows);
        ...
        public float[][] SolveForDiagonal(float[] diagonal)
        {
            if (diagonal == null)
        ...
        public float[] Solve(float[] value)
        {
            ...
            var VLU = new float[vrows][];
            for (var i = 0; i < vrows; i++)
            {
                VLU[i] = new float[urows];
                for (var j = 0; j < urows; j++)
                {
                    float sum = 0;
        ...
        public float[][] Inverse()
        {
            ...
            var X = new float[vrows][];
            for (var i = 0; i < vrows; i++)
            {
                X[i] = new float[Diagonal.Length];
        ...
        public float[] SolveTranspose(float[] value)
        {
            ...
            //(V x L*) x Ut x Y
            float[][] VL = Matrix.Dot(RightSingularVectors, Ls);
        ...
        public object Clone()
        {
            ...
            svd.m = m;
            svd.n = n;
            svd.Diagonal = (float[])Diagonal.Clone();
            svd.Ordering = (int[])Ordering.Clone();
            svd.swapped = swapped;
            if (LeftSingularVectors != null)
                svd.LeftSingularVectors = (float[][])LeftSingularVectors.MemberwiseClone();
            if (RightSingularVectors != null)
                svd.RightSingularVectors = (float[][])RightSingularVectors.MemberwiseClone();
