...
namespace ISynergy.Framework.Mathematics.Decompositions
{
    ...
    public sealed class CholeskyDecompositionD : ICloneable, ISolverMatrixDecomposition<decimal>
    {
                ...
                if (!lndeterminant.HasValue)
                {
                    ...
                    if (Diagonal != null)
                        for (var i = 0; i < Diagonal.Length; i++)
                            detD += Math.Log((double)Diagonal[i]);
        ...
        private void LLt()
        {
            Diagonal = Vector.Ones<decimal>(n);
            ...
            for (var j = 0; j < n; j++)
            {
                ...
                IsPositiveDefinite &= s > (decimal)1e-14 * Math.Abs(L[j, j]);
        ...
        private void LDLt()
        {
            ...
            for (var i = 0; i < n; i++)
            {
                ...
                // Use a tolerance for positive-definiteness
                IsPositiveDefinite &= v[i] > (decimal)1e-14 * Math.Abs(L[i, i]);
        ...
        public static CholeskyDecompositionD FromLeftTriangularMatrix(decimal[,] leftTriangular)
        {
            ...
            chol.robust = false;
            chol.Diagonal = Vector.Ones<decimal>(chol.n);
            return chol;
        }
        ...
        public object Clone()
        {
            ...
            clone.Diagonal = (decimal[])Diagonal.Clone();
