...
///         values of <c>λ</c>, must obey the identity <c>det(A - λ*B) = 0</c>.
///     </para>
///     <para>
...
///                 <description>
///                     <a
///                         href="http://en.wikipedia.org/wiki/Generalized_eigenvalue_problem#Generalized_eigenvalue_problem">
...
public sealed class JaggedGeneralizedEigenvalueDecomposition : ICloneable
{
    ...
    public JaggedGeneralizedEigenvalueDecomposition(double[][] a, double[][] b, bool sort = false)
    {
        ...
        n = a.Rows();
        RealAlphas = new double[n];
        ImaginaryAlphas = new double[n];
        Betas = new double[n];
        Eigenvectors = Jagged.Zeros(n, n);
        var A = a.Copy();
        var B = b.Copy();
        ...
        qzit(n, A, B, Constants.DoubleEpsilon, matz, Eigenvectors, ref ierr);

        // reduces the quasi-triangular matrix further, so that any
        // remaining 2-by-2 blocks correspond to pairs of complex
        // eigenvalues, and returns quantities whose ratios give the
        // generalized eigenvalues.
        qzval(n, A, B, RealAlphas, ImaginaryAlphas, Betas, matz, Eigenvectors);

        // computes the eigenvectors of the triangular problem and
        // transforms the results back to the original coordinate system.
        qzvec(n, A, B, RealAlphas, ImaginaryAlphas, Betas, Eigenvectors);
        ...
        if (sort)
        {
            ...
            Array.Sort(idx, (i, j) =>
            {
                if (Math.Abs(RealAlphas[i]) == Math.Abs(RealAlphas[j]))
                    return -Math.Abs(ImaginaryAlphas[i]).CompareTo(Math.Abs(ImaginaryAlphas[j]));
                return -Math.Abs(RealAlphas[i]).CompareTo(Math.Abs(RealAlphas[j]));
            });

            RealAlphas = RealAlphas.Get(idx);
            ImaginaryAlphas = ImaginaryAlphas.Get(idx);
            Betas = Betas.Get(idx);
    ...
    public int Rank
    {
        get
        {
            var tol = n * RealAlphas[0] * eps;

            var r = 0;
            for (var i = 0; i < RealAlphas.Length; i++)
                if (RealAlphas[i] > tol)
                    r++;
    ...
    public double[] RealAlphas { get; private set; }

    /// <summary>Returns the imaginary parts of the alpha values.</summary>
    public double[] ImaginaryAlphas { get; private set; }
    ...
    public double[] RealEigenvalues
    {
        ...
        {
            // ((alfr+i*alfi)/beta)
            var eval = new double[n];
            for (var i = 0; i < n; i++)
                eval[i] = RealAlphas[i] / Betas[i];
            return eval;
        }
    }
    ...
    public double[] ImaginaryEigenvalues
    {
        get
        {
            ...
            var eval = new double[n];
            for (var i = 0; i < n; i++)
                eval[i] = ImaginaryAlphas[i] / Betas[i];
            return eval;
        }
    }
    ...
    public double[][] DiagonalMatrix
    {
        get
        {
            ...
            for (var i = 0; i < n; i++)
            {
                ...
                x[i][i] = RealAlphas[i] / Betas[i];
                if (ImaginaryAlphas[i] > 0)
                    x[i][i + 1] = ImaginaryAlphas[i] / Betas[i];
                else if (ImaginaryAlphas[i] < 0)
                    x[i][i - 1] = ImaginaryAlphas[i] / Betas[i];
            }
    ...
    ///     For the full documentation, please check the original function.
    /// </remarks>
    private static int qzval(int n, double[][] a, double[][] b, double[] alfr, double[] alfi, double[] beta,
        bool matz, double[][] z)
    {
        int i, j;
        int na, en, nn;
        ...
        L490:
            if (an < (Math.Abs(e) + ei) * bn) goto L492;
            a1 = cz * b11 + szr * b12;
            a1i = szi * b12;
            a2 = szr * b22;
            a2i = szi * b22;
    ...
    ///     For the full documentation, please check the original function.
    /// </remarks>
    private static int qzvec(int n, double[][] a, double[][] b, double[] alfr, double[] alfi, double[] beta,
        double[][] z)
    {
        int i, j, k, m;
    ...
    public object Clone()
    {
        ...
        clone.ImaginaryAlphas = ImaginaryAlphas.Copy();
        clone.RealAlphas = RealAlphas.Copy();
        clone.Betas = Betas.Copy();
        clone.n = n;
        clone.Eigenvectors = Eigenvectors.Copy();
        return clone;
    }
