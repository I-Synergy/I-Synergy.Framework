...
public static class Special
{
    ...
    {
        var s = Normal.Inverse(0.5 * y + 0.5);
    ...
    /// <returns>
    ///     A vector with the same number of dimensions as <paramref name="input" />
    ...
    public static double BSpline(int n, double x)
    {
        ...
        for (var k = 0; k <= n + 1; k++)
        {
            var c = Binomial(n + 1, k) * Tools.TruncatedPower(x + (n + 1.0) / 2.0 - k, n);
            sum += positive ? c : -c; // Sum terms over k
            positive = !positive;
        }

        return 1.0 / Factorial(n) * sum; // Finally apply the 1/n! factor
    }
    ...
    {
        if (lnx == float.NegativeInfinity)
            return lny;
        if (lny == float.NegativeInfinity)
            return lnx;
