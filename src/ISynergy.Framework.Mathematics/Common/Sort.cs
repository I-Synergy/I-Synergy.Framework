namespace ISynergy.Framework.Mathematics.Common
{
    ...
    public static class Sort
    {
        ...
        ///     Insertion sort.
        /// </summary>
        public static int Insertion<TKey, TValue>(TKey[] keys, TValue[] items, Func<TKey, TKey, int> comparer,
            bool asc = true)
        {
        ...
        ///     Insertion sort.
        /// </summary>
        public static int Insertion<TKeys, TValue>(TKeys[] keys, TValue[] items, int first, int last,
            Func<TKeys, TKeys, int> comparer, bool asc = true)
        {
            Debug.Assert(last >= first);
        ...
        public static int Partition<TKey, TValue>(this TKey[] keys, TValue[] items, int first, int last,
            bool asc = true)
            where TKey : IComparable<TKey>
        {
            if (first >= last)
            ...
            var pivotIndex = pivot(keys, items, first, last - 1, asc);
            ...
            where T : IComparable<T>
        {
            ...
            var pivotIndex = pivot(keys, first, last - 1, asc);
            var pivotValue = keys[pivotIndex];
        ...
        /// <returns>The index of the new pivot.</returns>
        public static int Partition<TKey, TValue>(this TKey[] keys, TValue[] items, int first, int last,
            Func<TKey, TKey, int> compare, bool asc = true)
        {
            if (first >= last)
                return first;

            var pivotIndex = pivot(keys, items, first, last - 1, compare, asc);
            var pivotValue = keys[pivotIndex];
        ...
        public static int Partition<T>(this T[] items, int first, int last, Func<T, T, int> compare, bool asc = true)
        {
            ...
            var pivotIndex = pivot(items, first, last - 1, compare, asc);
        ...
        /// <returns>
        ///     If <paramref name="asc" /> is true, returns the n-th smallest element in
        ...
        ///     the array. Otherwise, returns the n-th largest.
        /// </returns>
        public static TKey NthElement<TKey, TValue>(this TKey[] keys, TValue[] items, int first, int last, int n,
            bool asc = true)
            where TKey : IComparable<TKey>
        {
            while (last - first >= INTROSORT_THRESHOLD)
