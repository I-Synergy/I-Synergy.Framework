...
namespace ISynergy.Framework.Synchronization.Core
{
    public abstract partial class BaseOrchestrator
    {
                             ...
                             CancellationToken cancellationToken, IProgress<ProgressArgs> progress)
        {
            ...
            await InterceptAsync(databaseChangesApplyingArgs, progress, cancellationToken).ConfigureAwait(false);
            ...
            if (hasChanges || context.SyncType != SyncType.Normal)
            {
                ...
                if (message.DisableConstraintsOnApplyChanges)
                {
                    ...
                    {
                        await InternalDisableConstraintsAsync(context, GetSyncAdapter(table, message.Setup), connection, transaction).ConfigureAwait(false);
                    }
                }
                ...
                if (context.SyncWay == SyncWay.Download && context.SyncType != SyncType.Normal && !message.SnapshoteApplied)
                {
                    ...
                    {
                        await InternalResetTableAsync(context, GetSyncAdapter(table, message.Setup), connection, transaction).ConfigureAwait(false);
                    }
                }
            ...
            CancellationToken cancellationToken, IProgress<ProgressArgs> progress)
        {
            ...
            var syncAdapter = GetSyncAdapter(schemaChangesTable, message.Setup);
            ...
            foreach (var batchPartInfo in bpiTables)
            {
                ...
                foreach (var syncRow in localSerializer.ReadRowsFromFile(fullPath, schemaChangesTable))
                {
                    ...
                    if (isBatch)
                    {
                        ...
                        if (batchRows.Count < Provider.BulkBatchMaxLinesCount)
                        {
                            batchRows.Add(syncRow);

                            if (rowsFetched < batchPartInfo.RowsCount && batchRows.Count < Provider.BulkBatchMaxLinesCount)
                                continue;
                        }
                ...
                if (conflictRows.Count > 0)
                {
                    ...
                    foreach (var conflictRow in conflictRows)
                    {
                        ...
                        var (conflictResolvedCount, resolvedRow, rowAppliedCount) =
                            await HandleConflictAsync(message.LocalScopeId, message.SenderScopeId, syncAdapter, context, conflictRow, schemaChangesTable,
                ...
                if (appliedRowsTmp > 0 || conflictsResolvedCount > 0)
                {
                    ...
                    await InterceptAsync(tableChangesBatchAppliedArgs, progress, cancellationToken).ConfigureAwait(false);
                }
            ...
            if (tableChangesApplied is not null)
            {
                ...
                await InterceptAsync(tableChangesAppliedArgs, progress, cancellationToken).ConfigureAwait(false);
            }
                                ...
                                CancellationToken cancellationToken, IProgress<ProgressArgs> progress)
        {
            ...
            (conflictApplyAction, conflictType, localRow, finalRow, nullableSenderScopeId) =
                await GetConflictActionAsync(context, localScopeId, syncAdapter, conflictRow, schemaChangesTable,
            ...
            if (conflictApplyAction == ApplyAction.Continue)
            {
                ...
                if (isMergeAction)
                {
                    ...
                    var isUpdated = await InternalApplyConflictUpdateAsync(context, syncAdapter, row, lastTimestamp, null, true, connection, transaction).ConfigureAwait(false);
            ...
            if (conflictApplyAction == ApplyAction.RetryWithForceWrite)
            {
                ...
                switch (conflictType)
                {
                    ...
                    case ConflictType.RemoteExistsLocalExists:
                        operationComplete = await InternalApplyConflictUpdateAsync(context, syncAdapter, conflictRow, lastTimestamp, nullableSenderScopeId, true, connection, transaction).ConfigureAwait(false);
                        rowAppliedCount = 1;
                        break;
                    ...
                    case ConflictType.RemoteIsDeletedLocalIsDeleted:
                        operationComplete = await InternalUpdateMetadatasAsync(context, syncAdapter, conflictRow, nullableSenderScopeId, true, connection, transaction).ConfigureAwait(false);
                        rowAppliedCount = 0;
                        break;
                    ...
                    case ConflictType.RemoteIsDeletedLocalExists:
                        operationComplete = await InternalApplyConflictDeleteAsync(context, syncAdapter, conflictRow, lastTimestamp, nullableSenderScopeId, true, connection, transaction);

                        // Conflict, but both have delete the row, so just update the metadata to the right winner
                        if (!operationComplete)
                        {
                            operationComplete = await InternalUpdateMetadatasAsync(context, syncAdapter, conflictRow, nullableSenderScopeId, true, connection, transaction);
                            rowAppliedCount = 0;
                ...
                if (!operationComplete)
                {
                    ...
                    Logger.LogError("Force update should always work..");
                }
            ...
            CancellationToken cancellationToken, IProgress<ProgressArgs> progress)
        {
            ...
            var interceptor = _interceptors.GetInterceptor<ApplyChangesFailedArgs>();
            ...
            {
                // Get the localRow
                localRow = await InternalGetConflictRowAsync(context, syncAdapter, localScopeId, conflictRow, schemaChangesTable, connection, transaction).ConfigureAwait(false);
                // Get the conflict
                var conflict = GetConflict(conflictRow, localRow);

                // Interceptor
                var arg = new ApplyChangesFailedArgs(context, conflict, resolution, senderScopeId, connection, transaction);
                await InterceptAsync(arg, progress, cancellationToken).ConfigureAwait(false);
            ...
            else
            {
                ...
                if (Logger.IsEnabled(LogLevel.Debug))
                {
                    ...
                    Logger.LogDebug(new EventId(SyncEventsId.ApplyChangesFailed.Id, "ApplyChangesFailed"), null, args);
                }
            }
