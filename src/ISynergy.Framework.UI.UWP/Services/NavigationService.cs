...
public class NavigationService : INavigationService
{
    ...
    private void SafeOnNavigatedTo(IViewModel viewModel)
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                _logger?.LogError(ex, "Error in OnNavigatedTo for {ViewModelType}", viewModel.GetType().Name);
            }
        }
    }
    ...
    private void SafeOnNavigatedFrom(IViewModel viewModel)
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                _logger?.LogError(ex, "Error in OnNavigatedFrom for {ViewModelType}", viewModel.GetType().Name);
            }
        }
    }
    ...
    /// </summary>
    /// <remarks>
    /// Uses <see cref="ISynergy.Framework.Mvvm.Abstractions.Commands.ICommandProvider"/> when available (AOT-safe path for .NET Native).
    ...
    /// The reflection fallback requires type metadata to be preserved in the UWP <c>Default.rd.xml</c> runtime directives file.
    /// </remarks>
    private bool HasRunningCommands(IViewModel viewModel)
    {
        ...
        // Reflection fallback — requires runtime directives in Default.rd.xml under .NET Native.
    ...
    /// Requires runtime directives (<c>Default.rd.xml</c>) under UWP .NET Native.
    /// Not compatible with NativeAOT or aggressive IL trimming.
    /// </summary>
