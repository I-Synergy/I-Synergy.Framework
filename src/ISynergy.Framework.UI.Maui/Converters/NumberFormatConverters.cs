...
public class DecimalNumberFormatter : IValueConverter
{
    ...
    {
        if (!string.IsNullOrEmpty(value?.ToString()) && decimal.TryParse(value?.ToString(), out decimal result)) // NOSONAR
        {
            return result;
        }
...
public class IntegerNumberFormatter : IValueConverter
{
    ...
    public object ConvertBack(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        if (!string.IsNullOrEmpty(value?.ToString()) && int.TryParse(value?.ToString(), out int result)) // NOSONAR
        {
            return result;
        }
